    ...
    override def read(jsValue: JsValue): Document = {
      ...
      json.getFields("id", "href", "type", "data") match {
        case Seq(JsString(id), JsString(href), JsString(typ), data: JsObject) =>
          ...
          val firstPublicationDate: Option[DateTime] = (json \ "first_publication_date").toOpt[String].map(new DateTime(_))
          val lastPublicationDate: Option[DateTime] = (json \ "last_publication_date").toOpt[String].map(new DateTime(_))
